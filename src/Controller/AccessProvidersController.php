...
//----------------------------------------Balance Transactions END-----------------------------------------
    ...
    public function edit()
    {
        ...
        // Set the Notifications email threshold and frequency
        if (!array_key_exists('notif_threshold', $this->request->data)) {
            $this->request->data['notif_threshold'] = 0;
        }
        if (!array_key_exists('notif_frequency', $this->request->data)) {
            $this->request->data['notif_frequency'] = 1;
        }
        $this->UserSettings->deleteAll(['user_id' => $ap_id, 'name' => 'notif_threshold']);
        $this->UserSettings->deleteAll(['user_id' => $ap_id, 'name' => 'notif_frequency']);
        $us_entity = $this->UserSettings->newEntity();
